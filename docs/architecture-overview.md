...
**Order violation detection:** `StepInstanceService.detectOrderViolations` runs on every completion and checks the completing step's **own** declared predecessors — the actions named in *its* `relatedAction`/`relatedSteps`, i.e. what it says it comes after. If any `must` predecessor already has a `step_instance` row that is still in a non-terminal, actionable state (`PENDING`/`DUE`/`OVERDUE`), an `ORDER_VIOLATION` deviation is raised on the just-completed step, naming the incomplete predecessor(s) in `metadata.incompletePrerequisites`.

> **Fixed 2026-09:** an earlier version of this check queried the wrong direction — it looked for steps whose `relatedSteps` named the *completing* step (i.e. its **successors**), not its predecessors, and flagged a violation whenever that successor hadn't happened yet. Since a step's successor is essentially never complete at the moment the step itself finishes (that's the normal, expected order), this produced a false `ORDER_VIOLATION` on almost every completion that had any mandatory successor — thousands of them accumulated in `deviation` before the fix. The corrected version reads the completing step's own `relatedSteps` (what it depends on), matching how `matcher-service` has always computed this via `cce-common-util`'s `PlanDefinitionParser.buildDependencyGraph`/`predecessorsOf` — compliance-service's own `PlanDefinitionParser` is a separate, service-local implementation and did not use that shared graph.
